...
public class MainActivity extends AppCompatActivity
    ...
    List<ContactsClass> listaMensajesClase;
    //para el api
    ...
    //PARA EL DIALOGO
    public void dialogPolitica(){
         final CheckBox seleccionPolicy;
        View viewx  = this.getLayoutInflater().inflate(R.layout.dialog_privacy_policies, null);
        numberDialog = new Dialog(this);
        numberDialog.setContentView(viewx);
        numberDialog.setTitle("POLÍTICA DE PRIVACIDAD");
        numberDialog.setCancelable(false);
        seleccionPolicy = (CheckBox) viewx.findViewById(R.id.checkBoxPolicy);
        btnRegister = (Button)viewx.findViewById(R.id.buttonAceptar);
        btnRegister.setEnabled(true);
        btnRegister.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                //guardar el check
                if(seleccionPolicy.isChecked()){
                    savePhoneCheck();
                    numberDialog.cancel();
                    checkPermission();
                }else{
                    Toast.makeText(MainActivity.this, "Acepte los terminos", Toast.LENGTH_LONG).show();
                }
            ...
            apiService.getNumberPbgGenero(MainActivity.dniAsesor).enqueue(new Callback<List<ContactsClass>>() {
                ...
                public void onResponse(Call<List<ContactsClass>> call, Response<List<ContactsClass>> response) {
                    if (response.isSuccessful()) {
                        ...
                        Cursor cursor;
                        for (ContactsClass smsx : listaMensajesClase) {
